...
questionnaire_data = {
    ...
    "2. 식욕 및 소화, 음수량": {
        ...
        "복부의 자각 증상": [
            {"code": "DIG_12", "text": "명치 아래가 답답하다"},
            {"code": "DIG_13", "text": "배가 가스가 찬 듯 빵빵하다"}
        ],
        "식욕 및 구역감": [
            {"code": "DIG_14", "text": "속이 울렁거린다"},
            {"code": "DIG_15", "text": "자주 토할 것 같다"},
            {"code": "DIG_16", "text": "입맛이 아예 없다"},
            {"code": "DIG_17", "text": "식욕을 참을 수 없다 (과식/폭식)"}
        ],
        "음수 (물 마시는 습관)": [
            {"code": "DIG_18", "text": "갈증이 심해 물을 많이 마신다"},
            {"code": "DIG_19", "text": "입은 마르는데 물은 안 마시고 싶다"},
            {"code": "DIG_20", "text": "물보다 커피나 음료수를 자주 마신다"},
            {"code": "DIG_21", "text": "술을 자주 마시거나, 한번에 많이 마신다"}
        ]
    },
    "3. 대변": {
        "변의 모양과 색 (최근 상태)": [
            {"code": "STL_01", "text": "음식물(밥알, 야채)이 그대로 보인다"},
            {"code": "STL_02", "text": "설사 혹은 풀어진 변"},
            {"code": "STL_03", "text": "돌처럼 단단한 변 (변비 경향)"},
            {"code": "STL_04", "text": "처음엔 단단하다가 뒤에는 풀어지는 변"},
            {"code": "STL_05", "text": "색이 회색이거나 희미하다"},
            {"code": "STL_06", "text": "색이 어둡고 검다"},
            {"code": "STL_07", "text": "피가 섞여 나온다"}
        ],
        "배변 감각 및 가스": [
    ...
    "7. 두면 및 이비인후과": {
        "코/호흡기 증상": [
            ...
            {"code": "ENT_02", "text": "맑거나 누런 콧물"},
            {"code": "ENT_03", "text": "코막힘"},
            {"code": "ENT_04", "text": "기침"},
            {"code": "ENT_05", "text": "가래"},
            {"code": "ENT_06", "text": "천식 기운이 있다"},
            {"code": "ENT_07", "text": "환절기 알러지성 비염"},
            {"code": "ENT_08", "text": "말을 조금만 해도 목소리가 빠르게 쉰다"},
    ...
    "9. 근골격계 통증": {
        "통증의 양상 (악화/완화 요인)": [
            ...
            {"code": "MSK_07", "text": "통증부위를 누르면 더 아프다"},
            {"code": "MSK_08", "text": "통증부위를 누르면 시원하다"}
        ],
        "부종 및 감각": [
    ...
    "10. 흉부 및 정신": {
        ...
        "정신/기억력": [
            ...
            {"code": "CHM_10", "text": "건망증이 있다"},
            {"code": "CHM_11", "text": "무기력/의욕저하가 반복된다"}
        ]
    },
    "11. 기타": {
        "체중": [
            {"code": "ETC_01", "text": "최근 3개월간 3kg 이상 체중 변화가 있다"}
        ]
    },
    "12. 남성": {
        "남성 기능": [
            {"code": "MEN_01", "text": "발기 부전 / 강직도 저하 증상이 있다"},
            {"code": "MEN_02", "text": "컨디션 저하 시 조루가 있다"},
            {"code": "MEN_03", "text": "성관계 시 통증이 발생한다"},
            {"code": "MEN_04", "text": "정액이 저절로 흘러나오는 증상이 있다"}
        ]
    },
    "13. 여성": {
        "생리/임신": [
            {"code": "WMN_01", "text": "생리주기가 불규칙하다"},
            {"code": "WMN_02", "text": "심한 생리통으로 진통제를 복용한다"},
            {"code": "WMN_03", "text": "생리양이 매우 많다"},
            {"code": "WMN_04", "text": "생리양이 매우 적다"},
            {"code": "WMN_05", "text": "출산 후 체중감량이 안된다"},
            {"code": "WMN_06", "text": "산후풍 (출산 후 관절통 등)"}
        ],
        "기타 여성 증상": [
            {"code": "WMN_07", "text": "생리 전 가슴/겨드랑이 통증이 있다"},
            {"code": "WMN_08", "text": "생리 전 감정 조절이 어렵다"},
            {"code": "WMN_09", "text": "냉대하 (양, 냄새, 색 이상)가 있다"},
            {"code": "WMN_10", "text": "잦은 질염이 발생한다"},
            {"code": "WMN_11", "text": "잦은 방광염이 발생한다"},
            {"code": "WMN_12", "text": "성교통이 심하다"},
...
# ==========================================================
    ...
    2. **2차 정밀 분석**: AI 시스템을 통해 현재 증상을 입체적으로 분석합니다.
    3. **3차 검증 및 생성**: 원장이 직접 설계하고 학습시킨 AI 모델로 1, 2차 내용을 종합 검증하여 분석 자료를 생성합니다.
    4. **최종 진단**: 내원 후 맥진, 복진, 호흡, 체형, 체질 등 상세 진찰을 통해 최종적인 치료 방향을 결정합니다.
...
# ==========================================================
    ...
    # [수정됨] 안내 문구 변경
    st.info("💡 항목을 체크하면 상세 입력란이 열립니다. **필수는 아니지만, 구체적으로 적어주시면 더 정확한 진단과 치료에 큰 도움이 되고, 환자분의 상태를 더 깊이 이해할 수 있습니다.**")
