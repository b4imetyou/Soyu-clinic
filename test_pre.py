...
# [페이지 1] 제출 완료 화면 (성공 시 여기로 점프)
if st.session_state['submitted']:
    # 완료 화면 스타일 (헤더 숨기고 여백 확보)
    st.markdown("""
    <style>
        ...
        /* 완료 화면 컨테이너 여백 확보 (모바일에서 가려짐 방지) */
        .block-container {
            padding-top: 100px !important; /* 위쪽 여백 충분히 */
        }
    </style>
    ...
    <div style="text-align: center; padding: 20px;">
        <h1 style="color: #0068c3; margin-bottom: 30px;">제출이 완료되었습니다!</h1>
        
        <div style="background-color: #f8f9fa; padding: 30px; border-radius: 15px; margin-bottom: 30px;">
            <h3 style="color: #333; margin: 0;">{default_name if default_name else "환자"} 님</h3>
            <p style="font-size: 18px; color: #555; margin-top: 10px;">
                <b>{reserved_date if reserved_date else ""}</b><br>
                진료 예약이 확인되었습니다.
            </p>
        </div>
...
# [페이지 3] 문진표 작성 화면
    ...
    <style>
        ...
        @media (min-width: 769px) {
            ...
            /* PC 로딩 메시지 위치 고정 (버튼 바로 아래) */
            .pc-loading-text {
                position: fixed;
                top: 180px; /* 버튼(120+50) 바로 아래 */
                left: 50%;
        ...
        @media (max-width: 768px) {
            ...
            .pc-loading-text { display: none !important; } /* 모바일은 그냥 하단에 표시 */
    ...
    if st.button("문진표 제출하기"):
        ...
        if not name or not cleaned_birth:
            ...
            # PC에서만 보이는 로딩 텍스트 (위치 고정)
            st.markdown("""
            <div class="pc-loading-text">
                🔄 AI 분석을 위해 변환 및 전송중...
            </div>
            """, unsafe_allow_html=True)
            
            # 모바일용 하단 텍스트 (버튼 아래에 표시)
            st.info("🔄 AI 분석을 위해 변환 및 전송중입니다... 잠시만 기다려주세요.")
